...
def parse_args():
    ...
    parser.add_argument("--output_csv", type=str, default="./outputs/inference_results.csv",
    ...
    with torch.no_grad():
        for i, sample in enumerate(tqdm(dataset, desc="Running inference")):
            ...
            result = {
                ...
                "parsed_prediction": json.dumps(parsed_json, ensure_ascii=False) if parsed_json else None,
                "ground_truth_parsed": json.dumps(gt_json, ensure_ascii=False) if gt_json else None,
            }
            ...
            if parsed_json:
                result.update({
                    ...
                    "pred_keywords": json.dumps(parsed_json.get("keywords", []), ensure_ascii=False),
                    "pred_categories": json.dumps(parsed_json.get("categories", []), ensure_ascii=False),
                    "pred_evidences": json.dumps(parsed_json.get("evidences", []), ensure_ascii=False),
                })
            ...
            if gt_json:
                result.update({
                    ...
                    "gt_keywords": json.dumps(gt_json.get("keywords", []), ensure_ascii=False),
                    "gt_categories": json.dumps(gt_json.get("categories", []), ensure_ascii=False),
                    "gt_evidences": json.dumps(gt_json.get("evidences", []), ensure_ascii=False),
                })
...
def calculate_metrics(results):
    ...
    for result in results:
        ...
        if (result.get("pred_claim_status") and 
            result.get("gt_claim_status") and
            result["pred_claim_status"] == result["gt_claim_status"]):
            claim_correct += 1
    ...
    metrics = {
        ...
        "valid_prediction_rate": (valid_predictions / total_samples * 100) if total_samples > 0 else 0,
        "claim_accuracy": (claim_correct / total_samples * 100) if total_samples > 0 else 0,
        "category_accuracy": (category_matches / total_samples * 100) if total_samples > 0 else 0,
    }
...
def main():
    ...
    logger.info(f"Valid predictions: {metrics['valid_predictions']} ({metrics['valid_prediction_rate']:.2f}%)")
