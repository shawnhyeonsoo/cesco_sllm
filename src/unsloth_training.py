    ...
    def on_step_end(self, args, state, control, model=None, **kwargs):
        ...
        if (
            ...
            for i, prompt in tqdm(enumerate(self.test_prompts)):
                # print("==GROUND TRUTH==")
                ground_truth = (
                    self.test_responses[i].replace(self.tokenizer.eos_token, "").strip()
                )
                # print(ground_truth)
                ...
                gt_claim = ground_truth["is_claim"]
                ...
                try:
                    ...
                    pred_mid_categories = [cat.split("__")[1] for cat in pred_category]
            ...
            logger.info(
                f"Claim Status Accuracy: {claim_accuracy}/{total_samples} ({claim_acc_percent:.2f}%)"
            )
            logger.info(
                f"Categories Accuracy: {category_accuracy}/{total_samples} ({category_acc_percent:.2f}%)"
            )
            logger.info(
            ...
            print("==============================================")
            print(
                f"Claim Accuracy: {claim_accuracy}/{total_samples} ({claim_acc_percent:.2f}%)"
            )
            print(
...
def parse_args():
    ...
    parser.add_argument(
        "--model_dir", type=str, default=os.environ.get("SM_MODEL_DIR", "./outputs")
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default=os.environ.get("SM_OUTPUT_DATA_DIR", "./outputs"),
    )
    ...
    if test_path:
        ...
        if test_path.endswith(".jsonl"):
            test_dataset = load_dataset(
                "json", data_files={"test": test_path}, split="test"
            )
        else:
...
def main():
    ...
    if args.use_wandb:
        ...
        run_name = (
            args.wandb_run_name
            or f"unsloth-{args.model_name.split('/')[-1]}-lr{args.learning_rate}"
        )
        wandb.init(
...
"""
    ...
    for idx in test_indices:
        ...
        if "### Instruction:" in formatted_text and "### Input:" in formatted_text:
            instruction_start = formatted_text.find("### Instruction:") + len(
                "### Instruction:"
            )
    ...
    logger.info(
        "Model will generate sample outputs every {} steps".format(args.save_steps)
    )
    trainer.train()
